...
router.post("/Timeline_TB", async (req, res) => {
    let result = await users.sequelize.query(
        `
        /* ++++++ */
        with tb1 as(
...
router.post("/AlarmTopic_time_test", async (req, res) => {
    try {
        let Result = await users.sequelize.query(
            `
            /* count time HH:mm:ss */
            with tb1 as(
              ...
              ,tb2 as ( 
              select  tb1.[topic] ,tb1.mfg_date,tb1.occur_new,tb1.[restored]
			  ,tb1.[hour],tb1.[sum],tb1.[mc_no],tb1.propcess
              ,[sum] [Time] ,[topic_masters].[responsible] 
               from tb1 left join [topic_masters]
			   on tb1.[topic] = [topic_masters].[Topic]
               ) 
			   ,tb3 as (select [topic],sum([sum])as [Time],[responsible] from tb2
			    where mfg_date = '2023-01-25'
               and [mc_no] = 'TB02'
			   and [responsible] = 'LINE TURNING'
               group by [topic],[responsible]
             )  
			 select top(3) [topic] ,[responsible]
              ,[Time]
              ,convert(varchar,DATEADD(s,[Time],0),8) as Alarm
              from tb3
              order by convert(varchar,DATEADD(s,[Time],0),8) desc 
