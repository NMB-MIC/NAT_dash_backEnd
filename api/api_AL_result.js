...
router.post("/AL_total_output_month", async (req, res) => {
    ...
    `--total_output by monthly -----------------------------------------------
      with tb1 as(
        SELECT 
          ...
          and [app_counter_accumoutput].pin = 'D306'  /*pin*/
    ...
    `/* CT */
      ...
      --convert(varchar, [app_counter_accumoutput].[registered_at], 23) as [registered_at]
            ...
            --,[app_counter_pin].pin
             ...
             and ([mc_name] = '' ` + command_mc + ` ) /*---*/
             and process = 'AL'/*---*/
             and [app_counter_accumoutput].[pin] ='D304'  /* AV CT Day */
...
router.post("/CT_month", async (req, res) => {
  ...
  let result = await users.sequelize.query(
    ...
    and ([mc_name] = '' ` + command_mc + ` ) /*---*/
    and process = 'AL'/*---*/
    and [app_counter_accumoutput].[pin] = 'D304'  /*  CT */
