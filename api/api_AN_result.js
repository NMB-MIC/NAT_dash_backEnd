...
router.post("/AN_total_output_month", async (req, res) => {
    ...
    `--total_output by monthly -----------------------------------------------
      with tb1 as(
        SELECT 
          ...
          and ([app_counter_accumoutput].pin = 'D3162' or [app_counter_accumoutput].pin = 'D3262'  )/*pin*/
...
// CT_Rear = D52 , CT_F =D51 /100
// UTL_Front = D416 , UTL_R = D436
// Yield_R = GD12  , yield_F = GD10 /10

router.post("/daily_CT_Rear", async (req, res) => {
    ...
    `/* CT */
      ...
      --convert(varchar, [app_counter_accumoutput].[registered_at], 23) as [registered_at]
            ...
            --,[app_counter_pin].pin
             ...
             and ([mc_name] = '' ` + command_mc + ` ) /*---*/
             and process = 'AN'/*---*/
             and [app_counter_accumoutput].[pin] ='D52'  /* Rear CT Day */
         group by [app_counter_accumoutput].[mfg_date] 
          `
  ); console.log("***************************CT AN by daily **********************", result);
  res.json({ result: result[0] });
});

router.post("/daily_CT_Front", async (req, res) => {
    ...
    `/* CT */
      ...
      --convert(varchar, [app_counter_accumoutput].[registered_at], 23) as [registered_at]
            ...
            --,[app_counter_pin].pin
             ...
             and ([mc_name] = '' ` + command_mc + ` ) /*---*/
             and process = 'AN'/*---*/
             and [app_counter_accumoutput].[pin] ='D51'  /* front CT Day */
         group by [app_counter_accumoutput].[mfg_date] 
          `
  );
  console.log("***************************CT_F AN by daily **********************", result);
  res.json({ result: result[0] });
});

router.post("/daily_UTL_Rear", async (req, res) => {
  var command_mc = "";
  for (let index = 0; index < req.body.selected_mc.length; index++) {
    command_mc = command_mc + ` or [mc_name] = '${req.body.selected_mc[index]}' `;
  }
  let result = await users.sequelize.query(
    `
      ...
      and ([mc_name] = '' ` + command_mc + ` ) /*---*/
      and process = 'AN'/*---*/
      and [app_counter_accumoutput].[pin] = 'D436'
      group by [app_counter_accumoutput].[mfg_date] 
        `
  );
  console.log("***************************utl AN by daily **********************", result);
  res.json({ result: result[0] });
});
router.post("/daily_UTL_Front", async (req, res) => {
  var command_mc = "";
  for (let index = 0; index < req.body.selected_mc.length; index++) {
    command_mc = command_mc + ` or [mc_name] = '${req.body.selected_mc[index]}' `;
  }
  let result = await users.sequelize.query(
    `
      ...
      and ([mc_name] = '' ` + command_mc + ` ) /*---*/
      and process = 'AN'/*---*/
      and [app_counter_accumoutput].[pin] = 'D416'
      group by [app_counter_accumoutput].[mfg_date] 
        `
  );
  console.log("***************************utl AN by daily **********************", result);
