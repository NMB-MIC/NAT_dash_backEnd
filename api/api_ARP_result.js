    ...
    try {
      ...
      as [hour]
          ...
          where  ([mfg_date] between '${req.body.date_start}' and '${req.body.date_end}' /*---*/ )
             ...
             and ([app_counter_pin].pin = 'D202' or  [app_counter_pin].pin = 'D204')
             )
             ,tb2 as(
...
router.post("/NG_ratio", async (req, res) => {
    try {
      ...
      ,[app_counter_accumoutput].[registered_at]
          ...
          and ([mc_name] =  '' `+ command_mc + ` )
       and ([app_counter_pin].[pin] = 'D210' or [app_counter_pin].[pin] = 'D212' or [app_counter_pin].[pin] = 'D200')
       and [accum_output] >0
            )
        ,tb1 as(
        select [mfg_date]
    ...
    SELECT [mfg_date]
        ...
        ,ISNULL([NG-],0) AS [NG-]
                    ...
                    for [name] in([Total],[NG+],[NG-])
                    ) as pvt)
    select [mfg_date]
    ,[Total]
    --,[NG+]
    ,cast(100*cast([NG+]as decimal(10,3))/[Total]as decimal(10,3)) as [NG_plus]
    --,[NG-]
    ,cast(100*cast([NG-]as decimal(10,3))/[Total]as decimal(10,3)) as [NG_minus]
    from tb_pvt
        ` )
        console.log({ result });
        res.json({
...
router.post("/ARP_total_output_month", async (req, res) => {
      ...
      `--total_output by monthly -----------------------------------------------
        with tb1 as(
          SELECT 
            ...
            and [app_counter_accumoutput].pin = 'D202'  /*pin*/
      ...
      `/* CT */
        ...
        --convert(varchar, [app_counter_accumoutput].[registered_at], 23) as [registered_at]
              ...
              --,[app_counter_pin].pin
               ...
               and ([mc_name] = '' ` + command_mc + ` ) /*---*/
               and process = 'ARP'/*---*/
               and [app_counter_accumoutput].[pin] ='D32'  /* AV CT Day */
