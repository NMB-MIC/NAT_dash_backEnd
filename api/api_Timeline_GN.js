...
// IRR
router.post("/mc_list_IRR", async (req, res) => {
    try {
        let result = await users.sequelize.query(
            `
            SELECT [mc_no]
            FROM [counter].[dbo].[mc_status]
            where [mc_no] like  'IR%_R%' 
            group by [mc_no]
            order by [mc_no]
            `
        );
...
router.post("/mc_status_log_IRR", async (req, res) => {
    try {
        let Result = await users.sequelize.query(
            `
            /* get status_log*/
...
router.post("/Timeline_IRR", async (req, res) => {
    var  command_process  = ``; 
    if (req.body.responsible == "All") {
        command_process = ``;
    } else {
...
router.post("/AlarmTopic_time_IRR", async (req, res) => {
    try {
        let Result = await users.sequelize.query(
            `
            /* count time HH:mm:ss */
...
router.post("/Stop_time_IRR", async (req, res) => {
    try {
        let Result = await users.sequelize.query(
            `
            with tb_a as(
